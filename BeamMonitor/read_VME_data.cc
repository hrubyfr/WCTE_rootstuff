...
void read_VME_data(TString filename = "data/VME_data/beamline_run1600_tuple_calib.root"){
	...
	vector<string>* pmt_id_name = nullptr;
	vector<int>* pmt_id = nullptr;
	vector<double>* pmt_charge = nullptr;

	tree->SetBranchAddress("beamline_id_name", &pmt_id_name);
	tree->SetBranchAddress("beamline_id", &pmt_id);
	tree->SetBranchAddress("beamline_qdc_charge", &pmt_charge);

	long nentries = tree->GetEntries();

	cout << "There are " << nentries << " entries" << endl;

	cout << "Loading first entry" << endl;
	tree->GetEntry(0);

	for (int i = 0; i < pmt_id->size(); i++){
		cout << pmt_id->at(i) << " " << pmt_id_name->at(i) << endl;
		if(pmt_id->at(i) >= 48) {
			cout << "Charge of " << pmt_id_name->at(i) << " is " << pmt_charge->at(pmt_id->at(i)) << endl;
		}
